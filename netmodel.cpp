...
crypto_type g_crypto_typ = crypto_type::crypto_xor;


template<size_t crypto_size_nonce,size_t crypto_size_secret>
class cCrypto{
public:
    cCrypto()
    {
    ...
    void crypto_gen(unsigned char * c,unsigned char * m,unsigned long long mlen)
    {
        switch (g_crypto_typ) {
        case crypto_type::crypto_xor:
            crypto_stream_xor(c,m,mlen,nonce.data(),secret_key.data());
            break;

        case crypto_type::crypto_salsa20:
            crypto_stream_salsa20_xor(c,m,mlen,nonce.data(),secret_key.data());
            break;

        case crypto_type::crypto_chacha20:
            crypto_stream_chacha20_xor(c,m,mlen,nonce.data(),secret_key.data());
        break;
...
private:
    std::array<unsigned char, crypto_size_nonce> nonce;
    std::array<unsigned char, crypto_size_secret> secret_key;
};

// Global my crypto
cCrypto<crypto_stream_NONCEBYTES,crypto_stream_KEYBYTES> mycrypto;
...
void marker_cmp(t_inbuf & inbuf,size_t bytes_transferred){
...
void fake_decrypt(size_t bytes_transferred,t_inbuf & inbuf){
    unsigned char bbb=0;
...
void avoid_warning(t_inbuf & inbuf){
...
void after_read(size_t bytes_transferred,t_inbuf & inbuf)
{
    if (cfg_test_crypto_task > 0) {
            fake_decrypt(bytes_transferred,inbuf);
    }
    else if (cfg_test_crypto_task < 0) {
            // this function always returns 0
            // https://nacl.cr.yp.to/stream.html
            mycrypto.crypto_gen(reinterpret_cast<unsigned char *>(&(inbuf.m_data[0])),reinterpret_cast<unsigned char *>(&(inbuf.m_data[0])),bytes_transferred);
...
void asioflow(with_strand< ThreadObject<asio::ip::udp::socket> > & mysocket,c_inbuf_tab & inbuf_tab, size_t inbuf_nr,
              std::mutex & mutex_handlerflow_socket)
{
    auto l_mt = [&mysocket, &inbuf_tab , inbuf_nr, & mutex_handlerflow_socket]()
            {
    ...
    if (cfg_mt_method == t_mt_method::mt_strand) {
            mysocket.get_strand().post(l_mt);
    }
    else if (cfg_mt_method == t_mt_method::mt_mutex) {
            mysocket.get_unsafe_assume_in_strand().get().get_io_service().post(l_mt);
    }
    else throw std::runtime_error("unsupported mt");
}

void algo_proces_done(with_strand< ThreadObject<asio::ip::udp::socket> > & mysocket,c_inbuf_tab & inbuf_tab, size_t inbuf_nr,
std::mutex & mutex_handlerflow_socket,t_inbuf & inbuf)
{
    ...
    auto l_done_mt=  [&mysocket, &inbuf_tab , inbuf_nr, & mutex_handlerflow_socket](const boost::system::error_code & ec, std::size_t bytes_transferred_again)
            {
    ...
    if (cfg_mt_method == t_mt_method::mt_mutex) {
            ...
            mysocket.get_unsafe_assume_in_strand() // in this block, we are using LOCKING so this is safe.
                    .get()
                    .async_receive_from( inbuf_asio , inbuf_tab.get(inbuf_nr).m_ep ,l_done_mt);
    }
...
void handler_receive(const e_algo_receive algo_step, const boost::system::error_code ec, std::size_t bytes_transferred,
        ...
        std::mutex & mutex_handlerflow_socket)
{
    _note("=====================00000000000000000000000000=======================");
        if (!! ec) {
        ...
        if ((algo_step==e_algo_receive::after_first_read) || (algo_step==e_algo_receive::after_next_read)) {
                ...
                // compare inbuf with marker
                marker_cmp(inbuf,bytes_transferred);

                after_read(bytes_transferred,inbuf);
                 // [asioflow]
                asioflow(mysocket,inbuf_tab,inbuf_nr,mutex_handlerflow_socket);
        } // first_read or next_read

        // ---
        else if (algo_step==e_algo_receive::after_processing_done) {
               algo_proces_done(mysocket,inbuf_tab,inbuf_nr,mutex_handlerflow_socket,inbuf);
        } // restart_read
